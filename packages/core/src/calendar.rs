//! Calendar event types and pure ICS (RFC 5545) parsing / recurrence expansion.
//!
//! Ported verbatim from `src-tauri/src/calendar/{mod,ics}.rs`, minus all
//! platform glue: the network fetch (`ureq`), the in-memory feed cache
//! (`once_cell`/`parking_lot`/`Instant`), and the macOS EventKit integration.
//! The host (Tauri shell or Obsidian plugin) fetches the `.ics` text and passes
//! it to [`events_in_window`]; the math here is byte-identical across targets.

use chrono::{DateTime, Duration, LocalResult, NaiveDate, NaiveDateTime, TimeZone, Utc};
use serde::{Deserialize, Serialize};
use std::collections::HashMap;

// ── Shared calendar shapes (from calendar/mod.rs) ───────────────────────────────
...
/// Parse `ics_text` and return the [`CalendarEvent`]s for `sub` that fall within
/// `[window_start, window_end]`, expanding RRULE/RDATE/EXDATE and honoring
/// RECURRENCE-ID overrides. This is the pure entry point the host calls after
/// fetching the feed body itself.
